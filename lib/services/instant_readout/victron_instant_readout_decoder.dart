...
class VictronInstantReadoutDecoder {
  ...
  Map<String, dynamic> _decodeBatteryMonitor(
    ...
    final int remainingMinutes = reader.readUnsigned(16);

    final int rawVoltage = reader.readSigned(16);

    final int alarm = reader.readUnsigned(16);

    final int rawAux = reader.readUnsigned(16);

    final int auxMode = reader.readUnsigned(2);

    final int rawCurrent = reader.readSigned(22);

    final int rawConsumedAh = reader.readUnsigned(20);

    final int rawSoc = reader.readUnsigned(10);

    final Map<String, dynamic> values = {
      'remainingMinutes': remainingMinutes == 0xffff ? null : remainingMinutes,
      'batteryVoltage': rawVoltage == 0x7fff ? null : rawVoltage / 100.0,
      'batteryCurrent': rawCurrent == 0x3fffff ? null : rawCurrent / 1000.0,
      'consumedAh': rawConsumedAh == 0xfffff ? null : -(rawConsumedAh / 10.0),
      'stateOfCharge': rawSoc == 0x3ff ? null : rawSoc / 10.0,
      'alarmCode': alarm,
      'auxMode': auxMode,
    };

    final double? voltage = values['batteryVoltage'] as double?;

    final double? current = values['batteryCurrent'] as double?;
    ...
    switch (auxMode) {
      case 0:
        values['starterVoltage'] = _signedValue(rawAux, 16) / 100.0;
        break;

      case 1:
        values['midpointVoltage'] = rawAux / 100.0;
        break;

      case 2:
        values['temperature'] = (rawAux / 100.0) - 273.15;
        break;
    }
  ...
  Map<String, dynamic> _decodeSolarCharger(
    ...
    final int chargeState = reader.readUnsigned(8);

    final int chargerError = reader.readUnsigned(8);

    final int rawBatteryVoltage = reader.readSigned(16);

    final int rawChargeCurrent = reader.readSigned(16);

    final int rawYieldToday = reader.readUnsigned(16);

    final int rawSolarPower = reader.readUnsigned(16);

    final int rawLoadCurrent = reader.readUnsigned(9);

    return {
      'chargeStateCode': chargeState == 0xff ? null : chargeState,
      'chargeState':
          chargeState == 0xff ? null : _chargeStateLabel(chargeState),
      'chargerError': chargerError == 0xff ? null : chargerError,
      'batteryVoltage':
          rawBatteryVoltage == 0x7fff ? null : rawBatteryVoltage / 100.0,
      'chargeCurrent':
          rawChargeCurrent == 0x7fff ? null : rawChargeCurrent / 10.0,
      'yieldTodayWh': rawYieldToday == 0xffff ? null : rawYieldToday * 10,
      'solarPower': rawSolarPower == 0xffff ? null : rawSolarPower,
      'loadCurrent': rawLoadCurrent == 0x1ff ? null : rawLoadCurrent / 10.0,
    };
  }
  ...
  Map<String, dynamic> _decodeAcCharger(
    ...
    try {
      ...
      final int deviceState = reader.readUnsigned(8);

      final int chargerError = reader.readUnsigned(8);

      final int rawVoltage1 = reader.readUnsigned(13);

      final int rawCurrent1 = reader.readUnsigned(11);

      final int rawVoltage2 = reader.readUnsigned(13);

      final int rawCurrent2 = reader.readUnsigned(11);

      final int rawVoltage3 = reader.readUnsigned(13);

      final int rawCurrent3 = reader.readUnsigned(11);

      final int rawTemperature = reader.readUnsigned(7);

      final int rawAcCurrent = reader.readUnsigned(9);

      // -----------------------------------------------------
      // DEVICE STATE
      // -----------------------------------------------------

      values['chargeStateCode'] = deviceState == 0xff ? null : deviceState;

      values['chargeState'] =
          deviceState == 0xff ? null : _chargeStateLabel(deviceState);

      // -----------------------------------------------------
      // CHARGER ERROR
      // -----------------------------------------------------

      values['chargerError'] = chargerError == 0xff ? null : chargerError;
      ...
      final double? voltage1 =
          rawVoltage1 == 0x1fff ? null : rawVoltage1 / 100.0;

      final double? current1 = rawCurrent1 == 0x7ff ? null : rawCurrent1 / 10.0;
      ...
      if (voltage1 != null && current1 != null) {
        values['power'] = voltage1 * current1;
      }
      ...
      values['outputVoltage2'] =
          rawVoltage2 == 0x1fff ? null : rawVoltage2 / 100.0;

      values['outputCurrent2'] =
          rawCurrent2 == 0x7ff ? null : rawCurrent2 / 10.0;
      ...
      values['outputVoltage3'] =
          rawVoltage3 == 0x1fff ? null : rawVoltage3 / 100.0;

      values['outputCurrent3'] =
          rawCurrent3 == 0x7ff ? null : rawCurrent3 / 10.0;
      ...
      values['temperature'] =
          rawTemperature == 0x7f ? null : rawTemperature - 40.0;

      // -----------------------------------------------------
      // AC INPUT CURRENT
      // -----------------------------------------------------

      values['acCurrent'] = rawAcCurrent == 0x1ff ? null : rawAcCurrent / 10.0;
    } on FormatException {
  ...
  Map<String, dynamic> _decodeOrionRaw(
    ...
    return {
      'raw': _bytesToHex(decrypted),
      'status': 'Orion payload detected; decoder mapping pending',
    };
  }
  ...
  Uint8List _decryptCtr({
    ...
    final Uint8List output = Uint8List(encrypted.length);
    ...
    while (offset < encrypted.length) {
      final Uint8List streamBlock = Uint8List(16);
      ...
      final int blockLength =
          encrypted.length - offset > 16 ? 16 : encrypted.length - offset;

      for (int index = 0; index < blockLength; index++) {
        output[offset + index] = encrypted[offset + index] ^ streamBlock[index];
      }
...
class _BitReader {
  ...
  int get remainingBits => (data.length * 8) - _bitIndex;
  int _bitIndex = 0;

  _BitReader(this.data);

  int readUnsigned(int bitCount) {
    if (_bitIndex + bitCount > data.length * 8) {
      throw const FormatException(
    ...
    for (int position = 0; position < bitCount; position++) {
      final int byteIndex = _bitIndex >> 3;

      final int bitInByte = _bitIndex & 7;

      final int bit = (data[byteIndex] >> bitInByte) & 1;
  ...
  int readSigned(int bitCount) {
    final int value = readUnsigned(bitCount);
